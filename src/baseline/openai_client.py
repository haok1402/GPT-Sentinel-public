...
from request_func import openai
...
class OpenAIClassifierClient:
    ...
    def __init__(self, config: Dict[str, str], header: Dict) -> None:
        self.config = config
        self.header = header
    ...
    async def rephrase_request(self, subset: str, humanTextUID: str, humanText: str, wait_time: float = 60.0) -> RequestResult:
        ...
        # Ready ... now Work!
        response, status_code = openai(humanTextUID, humanText, self.header)
    ...
    async def rephrase_dispatcher(self):
        ...
        self.worker_lock = asyncio.Semaphore(OpenAIClassifierClient.MAX_ASYNC_WORKER_COUNT)
        ...
        if output_path.exists():
            with open(output_path, "r") as f: lines = f.read().strip().split("\n")
            for line in lines:
...
if __name__ == "__main__":
    ...
    config_path = Path(CLI_PATH, "openai_config.json")
    # secret.json should contain
    # {
    #    "OpenAI-Organization": "...",
    ...
    secret = json.load(sec_handle := open(secret_path))
    cfg_handle.close()
    sec_handle.close()
    openai_headers.update(secret)

    client = OpenAIClassifierClient(config, openai_headers)
